...
const mumbaiEndpoint = 'https://polygon-mumbai.g.alchemy.com/v2/II-hu_srB3C0PDH1EAxuCrDM9bJSZFYI';

const provider = new HDWalletProvider({
  privateKeys: [wallet.privateKey],
  providerOrUrl: mumbaiEndpoint
});
...
async function setTransferFee(fee) {
  const nonce = await setUp();
  ...
  to: contractAddress,
  value: '0',
  data: contract.methods.setTransferFee(fee).encodeABI()
  };
  ...
  console.log("Transaction fee put.")
}
...
async function setTrustedForwarder(address) {
    ...
    const txParams = {
        ...
        to: contractAddress,
        value: '0',
        data: contract.methods.setTrustedForwarder(address).encodeABI()
    };
